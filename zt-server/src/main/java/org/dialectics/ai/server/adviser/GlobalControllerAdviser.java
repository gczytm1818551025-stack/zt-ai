...
public class GlobalControllerAdviser {
    ...
    // 应对文件预览或文件下载资源找不到，防止未匹配路径当作视图forward导致无限循环OOM
    @ExceptionHandler(NoResourceFoundException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND)
    public R<Object> handleNoResourceFoundException(NoResourceFoundException e, HttpServletRequest request) {
        String requestURI = request.getRequestURI();
        log.warn("请求地址'{}',资源未找到.", requestURI);
        return R.fail(404, "资源未找到: " + requestURI);
    }
